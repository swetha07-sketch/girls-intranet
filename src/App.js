...
styleTag.textContent = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-logo { font-family: 'Playfair Display', serif; font-style: italic; font-size: 1.5rem; color: var(--brown); }
  ...
  /* ── Create bar (like Facebook's "What's on your mind") ── */
  .create-bar { background: var(--card); border: 1px solid var(--border); border-radius: 20px; padding: 1rem 1.2rem; margin-bottom: 1.5rem; box-shadow: var(--shadow); display: flex; align-items: center; gap: 0.9rem; cursor: pointer; transition: box-shadow 0.2s; }
  ...
  .create-placeholder { flex: 1; padding: 0.6rem 1rem; border-radius: 999px; background: var(--cream); border: 1.5px solid var(--border); color: var(--taupe); font-size: 0.93rem; font-family: 'DM Sans', sans-serif; text-align: left; }
  ...
  /* ── Feed card (shared by posts and videos) ── */
  ...
  .mode-btn:hover, .mode-btn.selected { border-color: var(--rose); background: #fdf0ec; }
  ...
  @media (max-width: 540px) {
    ...
    .create-actions { width: 100%; justify-content: flex-end; }
  }
`;
...
export default function App() {
  ...
  const [feed, setFeed] = useState([]); // combined posts + videos
  const [loadingFeed, setLoadingFeed] = useState(true);
  const [uploadProgress, setUploadProgress] = useState(null);

  // sheet state
  const [showSheet, setShowSheet] = useState(false);
  const [sheetMode, setSheetMode] = useState(null); // null | 'post' | 'camera' | 'upload'
  ...
  useEffect(() => { if (authed) loadFeed(); }, [authed]);
  ...
  const loadFeed = async () => {
    ...
    const { data: posts } = await supabase
      .from("posts").select("*").gte("created_at", sevenDaysAgo).order("created_at", { ascending: false });
    ...
    const freshVideos = (files || [])
      .filter(f => f.name !== ".emptyFolderPlaceholder" && Date.now() - new Date(f.created_at).getTime() < week)
      .map(f => ({
        id: f.id,
        type: "video",
    ...
    const videoItems = freshVideos.map(v => ({ ...v, itemType: "video" }));
    const merged = [...postItems, ...videoItems].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    setFeed(merged);
  ...
  const startCamera = async (facing = facingMode) => {
    ...
    try {
      ...
      setCameraError("Couldn't access camera 📷 Please allow camera & microphone permissions in your browser settings, then try again.");
    }
  };
      ...
      <main className="main">
        ...
        {loadingFeed ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-text">Nothing yet! Be the first to post something ✨</p>
          </div>
        ) : (
          <div className="feed">
      ...
      {showSheet && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && closeSheet()}>
          <div className="sheet">
            ...
            {/* Step 2c — Camera preview after recording */}
